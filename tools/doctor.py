...
def check_spec_copy() -> tuple[bool | None, str]:
    default = main_checkout().parent / "heimathafen" / "vault" / "projects" / "create_brass_compass" / "spec"
    vault = Path(os.environ.get("BC_VAULT_SPEC", default))
    if not vault.exists():
        return None, f"spec copy: vault not present at {vault}; skipped (set BC_VAULT_SPEC to check)"
