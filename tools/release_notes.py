...
def main() -> None:
    ...
    checksum = Path(f"dist/create_brass_compass-{version}.jar.sha256").read_text().split()[0]
    print(f"# Create Fly: Brass Compass {version}\n")
    print(body)
    print(f"\n## Files\n\n`create_brass_compass-{version}.jar`, SHA-256 `{checksum}`.")
