...
Start from `create_civilization`'s `JobBoardMenu`/`JobBoardScreen` pair: a `MenuBase` with no slots whose `clickMenuButton` chooses a row, a `MenuProvider` writing the listing into the opening buffer, and an `AbstractSimiContainerScreen` over the stock-keeper header, body and footer textures with a scroll offset.
